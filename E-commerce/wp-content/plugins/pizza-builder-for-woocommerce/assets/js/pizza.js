(function ($) {
  ...
  function calculateEvPizza() {
    ...
    if ($("form.variations_form").length > 0) {
      ...
      const templateEvLayers = () => {
		...
		let index = $(".pizza-fancybox-layers .pizza-layers-selected__item").index(
  ...
  class EV_Pizza_Builder {
	constructor(stepId) {
		...
		this.step = 1;
		this.shownSteps = [];
		this.required = this.data.data.components.map(c => ({step: c.step, required: c.required == 1 ? true : false}));
		//events
			...
			opts: {
				afterShow: (instance) => {
					...
					if (FOOD_FRONT_DATA.tippy_enabled) {
						tippy("[data-tippy-content]", {appendTo : instance.$refs.container[0],zIndex: 99999});
					  }
	...
	submit(e) {
		...
		.then(res => {
			...
			if(res.cart_hash) {
				this.successContainer.html('<svg width="67" height="67" viewBox="0 0 67 67" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="33.5" cy="33.5" r="33.5" fill="#71DB1A"/><path d="M45.4639 20.3394L30.0023 46.8724L20.3389 35.0799" stroke="white" stroke-width="3" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/></svg><p>Done!</p>')
